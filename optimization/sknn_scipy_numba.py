...
@njit
def sort_neighbors(neighbors, similarity, k):
    sort_index = np.argsort(-similarity)
    neighbors = neighbors[sort_index][:k]
    similarity = similarity[sort_index][:k]
    return neighbors, similarity

@njit
def nonzero_index(filter_on):
    index = filter_on.nonzero()
    return index[0]

@njit
def sort_score(candidates, scores):
...
class SessionKNN: 
    ...
    def predict( self, playlist, tracks, playlist_id=None, artists=None, num_hidden=None ):
        ...
        candidates, scores = sort_score(candidates, scores)
        end = time.time()
        s_time = end - start
    ...
    def calc_similarity(self, tracks, possible_neighbors):
        ...
        indices = nonzero_index((similarity > threshold))
    ...
    def find_neighbors(self, tracks):
        ...
        # descending sort
        possible_neighbors, similarity = sort_neighbors(possible_neighbors, similarity, self.k)
    ...
    def score_items(self, neighbors, similarity):
        ...
        candidate_index = nonzero_index(score_vector)
