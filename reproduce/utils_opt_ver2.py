...
@njit
def filter_lists(train, test):
    train_playlists_id = train['playlists'].playlist_id.values
    test_playlists_id = test['playlists'].playlist_id.unique()
    train_actions_playlist_id = train['actions'].playlist_id.values
    train_tracks_track_id = train['tracks'].track_id.values
    train_actions_track_id = train['actions'].track_id.values
    train_actions_artist_id = train['actions'].artist_id.values

    playlists_filter = ~np.in1d(train_playlists_id, test_playlists_id)
    actions_filter = np.in1d(train_actions_playlist_id, train_playlists_id)
    tracks_filter = np.in1d(train_tracks_track_id, train_actions_track_id)
    artists_filter = np.in1d(train['artists'].artist_id, train_actions_artist_id)

    return playlists_filter, actions_filter, tracks_filter, artists_filter
...
def load_dataset( folder_train, folder_test, feather=False ):
    ...
    # filter lists
    # playlists in train but not in test
    playlists_filter, actions_filter, tracks_filter, artists_filter = filter_lists(train, test)
    train['playlists'] = train['playlists'][playlists_filter]
    train['actions'] = train['actions'][actions_filter]
    train['tracks'] = train['tracks'][tracks_filter]
    train['artists'] = train['artists'][artists_filter]
    
    print( ' -- filtered in: {}s'.format( time.time() - tstart ) )
        
    # test data has tracks not in train data
